...
class WebsiteMonitor:
    ...
    def check_website(self, url: str, regex_pattern: Optional[str] = None) -> Dict[str, Any]:
        ...
        for attempt in range(self.retry_limit):
            try:
                ...
                result['check_details']['dns_lookup_time'] = response.elapsed.total_seconds() * 1000
                result['check_details']['total_network_time'] = response_time_ms
                result['check_details']['content_size_bytes'] = len(response.content)
                result['check_details']['headers'] = dict(response.headers)
                
                # Enhanced regex pattern logging with color coding
                if regex_pattern and result['success']:
                    logger.info(f"\033[95mApplying regex pattern: '{regex_pattern}' to response body\033[0m")
                    ...
                    if match:
                        ...
                        logger.info(f"\033[92mRegex pattern matched at position {match.span()}\033[0m")
                    else:
                        logger.info(f"\033[93mRegex pattern '{regex_pattern}' not found in content\033[0m")
                        result['success'] = False
                        result['failure_reason'] = f"Regex pattern '{regex_pattern}' not found"
                
                break  # Exit retry loop on success
                
            except RequestException as e:
                logger.info(f"\033[93mAttempt {attempt + 1}/{self.retry_limit} failed for {url}: {str(e)}\033[0m")
                ...
                logger.error(f"\033[91mUnexpected error checking {url}: {str(e)}\033[0m")
        ...
        if result['success']:
            print(f"\033[97;42m WEBSITE CHECK \033[0m \033[1;92m✓ SUCCESSFUL\033[0m \033[1;94m{url}\033[0m - "
